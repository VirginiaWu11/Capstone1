...
from constants import BMI_LOW_NORMAL, BMI_HIGH_NORMAL, ACTIVITY_LEVELS, PLANS, BASE_API_URL
...
class UserFoodService:
    ...
    def query_user_bmi_information(cls, user):
        ...
        user_bmi_query = BMI.query.order_by(BMI.date.desc()).filter_by(user_id=user.id).limit(7).all()
    ...
    def get_bmi_information(cls, date_bmi_weight_entries_ascending, user):
        ...
        for t in date_bmi_weight_entries_ascending:
            ...
            weight_lows_normal.append(BMI.calculate_normal_low_weight_by_height(int(user.height)))
            weight_highs_normal.append(BMI.calculate_normal_high_weight_by_height(int(user.height)))
        return {
    ...
    def get_user_goal_calories_in(cls, user_calories_out, user):
        #### Goal Calories In
        user_goal_calories_in = [calories_out+PLANS[user.diet_plan] for calories_out in user_calories_out]
